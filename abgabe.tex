...
\fcolorbox{gray}{gray!10}{%  <-- Erzeugt einen Kasten (benötigt \usepackage{xcolor})
    \parbox{0.9\columnwidth}{% <-- Erlaubt Textumbruch innerhalb der Box
        ...
        \textbf{Quellen:}\par % \par für neue Zeile
        % \fullcite druckt den gesamten Eintrag aus der .bib-Datei
        \textbf{1. Erklärt die Grundbegriffe der Ethik und Tugend und ordnet sie in den Arbeitsalltag ein. }\par
        \fullcite{renz2022} \par\medskip

        \textbf{2. Definition verantwortung}\par
        \fullcite{gabler_verantwortung} \par\medskip

        \textbf{3. Eine Masterarbeit über den ethischen Entscheidungsfindungsprozess in der Pflege. Enthält Informationen über Grundbegriffe der Ethik, bevor die Arbeit in das Thema Pflege einsteigt. Könnte eine gute Grundlage für unsere Ausarbeitung liefern.        }\par
...
\fcolorbox{gray}{gray!10}{%  <-- Erzeugt einen Kasten (benötigt \usepackage{xcolor})
    \parbox{0.9\columnwidth}{% <-- Erlaubt Textumbruch innerhalb der Box
        ...
        \textbf{Quellen:}\par % \par für neue Zeile
        % \fullcite druckt den gesamten Eintrag aus der .bib-Datei
        \textbf{1. Definition whistleblowing}\par
        \fullcite{ihk-whistleblowing} \par\medskip
        \textbf{2. Bedeutung whistleblowing in der wirtschaft}\par
        \fullcite{lehnart2023} \par\medskip

        \textbf{3. Prägnante begriffserklärung Whistleblowing mit unterteilung in verschiedene Szenarien}\par
...
\fcolorbox{gray}{gray!10}{%  <-- Erzeugt einen Kasten (benötigt \usepackage{xcolor})
    \parbox{0.9\columnwidth}{% <-- Erlaubt Textumbruch innerhalb der Box
        \small % Kleinere Schrift für die Notiz
        \textbf{Quellen für dieses Kapitel:}\par % \par für neue Zeile
        % \fullcite druckt den gesamten Eintrag aus der .bib-Datei
...
\fcolorbox{gray}{gray!10}{%  <-- Erzeugt einen Kasten (benötigt \usepackage{xcolor})
    \parbox{0.9\columnwidth}{% <-- Erlaubt Textumbruch innerhalb der Box
        ...
        \textbf{Quellen:}\par % \par für neue Zeile
        % \fullcite druckt den gesamten Eintrag aus der .bib-Datei
        \textbf{1. Beschäftigt sich damit wie man eine gesunde “Compliance” kultur etabliert und pflegt durch “tools”}\par
        \fullcite{veit2021} \par\medskip
        \textbf{2. Das erste Kapitel möchte Business Ethiks in den Kontext von Kants Moral "Constructions" aufbauen. Part 3 beleuchtet wichtige Aspekte der Ethik im Unternehmensbereich }\par
        \fullcite{robinson2024} \par\medskip
        \textbf{3. Ausarbeitung des Themas Individualethik. Ethische Entscheidungsfindung, ethische Dilemma und ethische Kompetenzen werden behandelt. Auch werden ethische Barrieren von individuen beleuchtet }\par
        \fullcite{imzuwi-verantwortung} \par\medskip
        \textbf{4. Kurzer Blogpost über Integrität allgemein}\par
...
\fcolorbox{gray}{gray!10}{%  <-- Erzeugt einen Kasten (benötigt \usepackage{xcolor})
    \parbox{0.9\columnwidth}{% <-- Erlaubt Textumbruch innerhalb der Box
        ...
        \textbf{Quellen:}\par % \par für neue Zeile
        % \fullcite druckt den gesamten Eintrag aus der .bib-Datei
        \textbf{1. Interessant, dass es für Berufsgruppen einen Ethikcode gibt.}\par
